{
  pkgs,
  lib,
  rebuildCommand ? "home-manager switch --flake ~/.config/nix",
  ...
}:
...
in
{
  ...
  programs.tmux = {
    ...
    extraConfig = ''
      ...
      bind -T copy-mode-vi y send-keys -X copy-pipe-and-cancel "${tmuxCopyCommand}"
      bind P paste-buffer
      bind -T copy-mode-vi MouseDragEnd1Pane send-keys -X copy-pipe-and-cancel "${tmuxCopyCommand}"
